...
| **[IN PROGRESS]** 23.7 | **Entity signature extraction** | For functions/methods/classes, extract and store the signature (name + params + return type) separately from the body. Signatures are the ONE piece of "code" Synapses provides — not implementations, just interfaces. Addresses FeatureBench D2 failure (hallucinated interfaces). | Medium |
