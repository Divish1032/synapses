...
```
  ...
  Sprint 27: Intelligence & Adaptation . 6 tasks — Auto-adaptive context + doc ingestion + learned patterns
```
...
**Outcome:** SWE-bench proved too easy — 20/20 without Synapses tools. Benchmark removed (along with FeatureBench) in favor of ContextBench as the primary retrieval quality metric.
...
## Sprint 27: Intelligence & Adaptation

**Goal:** Make Synapses a smarter silent worker — auto-detect project lifecycle, adapt context delivery, surface reactive guidance, ingest non-code documents, and learn from agent behavior patterns. Inspired by AnythingLLM analysis (pinned context, chat modes, doc ingestion) but designed for Synapses' silent-worker philosophy.

| # | Task | What | Effort | Impact |
|---|------|------|--------|--------|
| 27.1 | **SDLC auto-detection via work ledger** | Infer project phase (planning/development/testing/review/deployment) from tool-call patterns + file signals. Pure heuristic, no LLM. Auto-adjusts QualityMode. Explicit SetSDLCPhase always overrides. | Medium | High |
| 27.2 | **Phase-aware CarveConfig defaults** | Context delivery adapts to SDLC phase — planning gets broader depth + architectural edges, testing gets stricter relevance, review favors callers for blast radius, deployment minimizes scope. Applied after intent config (secondary modifier). | Low | High |
| 27.3 | **Reactive tool guidance in responses** | Key tool responses (get_context, search, validate) carry a `suggested_next` field — context-aware hints based on entity properties (fanIn, arch rules), SDLC phase, and session history. Suppressed for tools called 3+ times. Max 2 suggestions. | Medium | Medium |
| 27.4 | **Async doc-to-graph pipeline (PDF/DOCX)** | Watcher detects PDF/DOCX files, extracts text (pdftotext + Go fallback), splits into sections, creates graph nodes with EXPLAINS/DOCUMENTED_BY edges. Brain enriches asynchronously via P2 scheduler queue. | High | Medium |
| 27.5 | **Auto-pinning via co-access patterns** | Session-end analysis records entity co-access patterns in context_patterns table. BFS carving injects co-accessed entities as virtual neighbors (confidence ≥ 0.4, capped at 5). Decay 0.95× per non-reinforced session. | Medium | High |
| 27.6 | **Cross-project memory transfer** | Recall memories across projectID boundaries for pattern reuse between projects. Federation Phase 4 prerequisite. Requires cross-project memory query API + confidence scoring for pattern applicability. | High | Medium |

**Dependencies:** 27.2 depends on 27.1. All others independent.
...
| 6 | **Sprint 27: Intelligence & Adaptation** | Deepen the moat. Auto-adaptive context delivery + doc ingestion + learned patterns. |
