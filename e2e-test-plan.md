...
_Reusable across versions. Run this whenever you want to benchmark how well the 3-leg system performs._
...
curl -s -X POST http://localhost:11435/v1/enrich \
  -H "Content-Type: application/json" \
  -d '{"root_id":"test-e2e-001","root_name":"CarveEgoGraph","root_type":"function","all_node_ids":["test-e2e-001"],"callee_names":[],"caller_names":[],"related_names":[]}'
```
...
- **Pass**: returns `phase` and `quality_mode` fields
...
## E2E Test Results — v0.6.1 — 2026-03-04

### System Versions

- synapses: v0.4.1-23-g1f8f229 (git-computed; manual tag not bumped ⚠️)
- synapses-intelligence: v0.6.1 ✅
- synapses-scout: v0.0.5 (pyproject.toml); server reports "0.0.1" — version string not updated ⚠️

### Environment

- macOS (darwin), Apple Silicon M-series, 18 GB RAM, Metal GPU
- Ollama 0.17.6 (IMPORTANT: new API version — see BUG-I-NEW-A below)
- Models: qwen3.5:0.8b (ingest/prune), qwen3.5:2b (guardian), qwen3.5:4b (enrich), qwen2.5-coder available
- brain.json: db_path=/Users/itachi/.synapses/brain.sqlite (absolute ✅), timeout_ms=30000
...
| Check              | Pass/Fail  | Score      | Notes                                                                                                                                                                                                                  |
| ------------------ | ---------- | ---------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| L1-A session_init  | ⚠️ Partial | 5          | Tool IS in binary (confirmed via strings); not in Claude Code session tool list (session started before binary fully propagated / scout not configured at startup → conditional tool set). New binary required.        |
| L1-B find_entity   | ⚠️ Partial | 8          | Found in 1 call with full doc+signature ✅. Test functions still rank above Graph.CarveEgoGraph (rank 5) — regression from v0.6.0 which ranked it first.                                                               |
| L1-C token savings | ⚠️ Partial | 6          | format=compact in new binary (digest.go confirmed). JSON response measured: ~1800 tokens. Compact estimated ~80-150 tokens → ~92% reduction. Cannot test live (running session uses binary without compact).           |
| L1-D call chain    | ⚠️ Partial | 7          | "no call chain exists" for cross-binary cmdStart→Builder.Build. No explanation of cross-binary boundary. Same as v0.6.0.                                                                                               |
| L1-E search        | ✅ Pass    | 8          | "rate limiting" → 1 relevant hit ✅. "BFS carver" → 2 hits ✅ (camelWords + sanitizeFTSQuery doc both relevant). Improvement over v0.6.0's 1 hit.                                                                      |
| L1-F impact        | ✅ Pass    | 9          | 13 nodes in 3 tiers (direct/indirect/peripheral) ✅. Compact output (no overflow). Files listed. BUG-NEW-03 fix confirmed.                                                                                             |
| L1-G validate_plan | ⚠️ Partial | 4          | Returns "no matching node in graph" warning — file path is relative but graph uses absolute paths. Rule loaded (hot-reload picked up synapses.json) but path resolution fails in validate_plan. No violation detected. |
| L1-H working_state | ✅ Pass    | 9          | Correctly shows all recent changes incl. builds and synapses.json creation ✅. suggested_tools present ✅.                                                                                                             |
| **Leg 1 Score**    |            | **6.9/10** | Weighted: (5+8+12+7+8+9+4+9)/9                                                                                                                                                                                         |

### Leg 2 — Intelligence Sidecar

| Check                  | Pass/Fail  | Score      | Notes                                                                                                                                                                |
| ---------------------- | ---------- | ---------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| L2-A health            | ✅ Pass    | 10         | available:true, model:qwen3.5:4b, status:ok                                                                                                                          |
| L2-B ingest            | ❌ Fail    | 2          | "empty response from LLM" after 13.8s. Root cause: BUG-I-NEW-A — Ollama 0.17.6 uses think:bool API field; /no_think prefix returns empty response with qwen3.5:0.8b. |
| L2-C enrich            | ❌ Fail    | 2          | API field names changed (root_name required, not name). Timed out at 30s with llm_used:false. Same BUG-I-NEW-A root cause.                                           |
| L2-D context packet    | ⚠️ Partial | 6          | Fast path returns phase/quality_gate/phase_guidance ✅. packet_quality=0.0 (cold brain, no summaries). entity_name empty (node_id not resolved).                     |
| L2-E explain violation | ❌ Fail    | 2          | "unexpected end of JSON input (raw: \"\")" after 20.4s. Same BUG-I-NEW-A.                                                                                            |
| L2-F SDLC              | ✅ Pass    | 9          | phase=development ✅. Field renamed: quality_mode (was mode) — minor breaking change.                                                                                |
| L2-G prune             | ❌ Fail    | 4          | Endpoint responds in 14.8s ✅ (graceful LLM failure). pruned_length=original_length=247 — no compression. Same BUG-I-NEW-A. Fail-silent handled correctly.           |
| **Leg 2 Score**        |            | **4.5/10** | Weighted: (10+4+4+12+2+9+4)/10. Root cause: single bug BUG-I-NEW-A blocks all LLM calls.                                                                             |

### Leg 3 — Scout

| Check               | Pass/Fail  | Score      | Notes                                                                                                                                                       |
| ------------------- | ---------- | ---------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- |
| L3-A health         | ✅ Pass    | 9          | status:ok, intelligence_available:true ✅. version:"0.0.1" (server string not bumped, pyproject.toml says 0.0.5 ⚠️).                                        |
| L3-B search         | ⚠️ Partial | 8          | 3 hits in 4.76s ✅. hits key ✅. Slower than v0.6.0 (4.76s vs 1.48s) — network variability.                                                                 |
| L3-C fetch          | ⚠️ Partial | 7          | content_md: 29943 chars ✅, 1.15s ✅. source: None — source field missing from response.                                                                    |
| L3-D distillation   | ❌ Fail    | 2          | distilled:False, fragment:None. Brain's prune fails (BUG-I-NEW-A) → scout distillation pipeline fails. Cascading failure.                                   |
| L3-E cache          | ✅ Pass    | 10         | 18ms cache hit ✅ (99% faster). cached:True ✅.                                                                                                             |
| L3-F MCP web_search | ❌ Fail    | 0          | web_search not in Claude Code session tool list. Scout was not configured at session startup → tools may not have been registered or session needs restart. |
| L3-G web_annotate   | ❌ Fail    | 0          | Same as L3-F — web_annotate not in session tools.                                                                                                           |
| **Leg 3 Score**     |            | **4.6/10** | Weighted: (9+16+7+4+10+0+0)/10. L3-F/G blocked by session setup issue.                                                                                      |
...
| Metric                                          | Value                                                               |
| ----------------------------------------------- | ------------------------------------------------------------------- |
| get_context compact vs json                     | ~92% reduction (~150 vs ~1800 tokens, estimated from code analysis) |
| get_impact (Graph.AddNode)                      | 13 entities across 3 files, 1 call — no manual equivalent           |
| search "BFS carver" vs grep                     | 2 precise hits vs 200+ files grep noise                             |
| Estimated tokens saved per session              | ~12,000–18,000 tokens                                               |
| Estimated monthly API cost saved (100 sessions) | ~$3.60–$5.40/month                                                  |

### Final Rating

```
Leg 1 (Core):         6.9/10
Leg 2 (Intelligence): 4.5/10  ← BUG-I-NEW-A (Ollama 0.17.6 API change)
Leg 3 (Scout):        4.6/10  ← cascading from L2 + session tools issue
Token Score:          1.0 (92% reduction, capped)

Composite = (0.69 × 0.35) + (0.45 × 0.30) + (0.46 × 0.25) + (1.0 × 0.10)
          = 0.2415 + 0.135 + 0.115 + 0.10 = 0.5915
```

**Composite Score: 5.9 / 10 — Acceptable**
vs v0.6.0: 8.3 → 5.9 (−2.4 points)

> **Key insight:** The regression is caused by a SINGLE bug (BUG-I-NEW-A) that blocks all LLM calls.
> Ollama 0.17.6 changed the thinking control API from `/no_think` prompt prefix to `think: false`
> JSON field. Once fixed, all L2 tests should pass and L3-D (distillation) recovers automatically.
> Without BUG-I-NEW-A, projected score: 8.0–8.5/10 (Excellent range).

### Bugs Found

| ID             | Severity | Status | Description                                                                                                                                                                                                                                                                  |
| -------------- | -------- | ------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| BUG-I-NEW-A    | Critical | FIXED  | Ollama 0.17.6: thinking control changed from `/no_think` prefix to `think: false` API field. Fixed in `synapses-intelligence/internal/llm/ollama.go` by adding `Think *bool` to `ollamaRequest` struct. Verified: LLM now responds correctly without the empty-response bug. |
| BUG-I-NEW-B    | Medium   | FIXED  | enrich API field names changed: test plan uses `node_id`/`name`/`summary`, actual API requires `root_id`/`root_name`/`root_type`. Test plan updated.                                                                                                                         |
| BUG-I-NEW-C    | Low      | FIXED  | L2-F: SDLC field renamed from `mode` to `quality_mode` — minor undocumented breaking change. Test plan updated to use `quality_mode`.                                                                                                                                        |
| BUG-S-NEW-D    | Low      | FIXED  | L3-C: `source` field was not persisted to `scout_cache` SQLite table. Added `source` column to schema + migration in `cache.py`. GET from cache now returns `"trafilatura"` / `"browser"` correctly.                                                                         |
| BUG-S-NEW-E    | Low      | FIXED  | L3-A: scout server now reports `version: 0.0.5` — fixed by reinstalling editable package so `importlib.metadata` picks up updated `pyproject.toml`.                                                                                                                          |
| BUG-SESSION-01 | Medium   | FIXED  | web_search, web_fetch, web_annotate, session_init not in Claude Code session tool list. Root cause documented: `synapses.json` must exist at MCP root before starting Claude Code. Test plan updated with prerequisite note.                                                 |
| BUG-L1G-01     | Low      | FIXED  | `validate_plan` path resolution fixed in `graph.go`: `FindByFile` now uses `strings.HasSuffix(n.File, "/"+filePath)` so callers can pass relative paths and they match against absolute paths stored in the graph.                                                           |
| CONFIG-01      | Medium   | FIXED  | `synapses.json` discovery: added `FindConfigDir` in `config.go` to walk up from `--path` until the config is found (like `.git` discovery), supporting partial-repo indexing natively.                                                                                       |

### What Improved vs v0.6.0

- Brain: BUG-I02 (db_path tilde) fixed ✅
- Brain: BUG-I01 (WithThinking hardcoded) fixed ✅ — but now blocked by Ollama 0.17.6 API change
- Brain setup: Apple Silicon / Metal GPU detected ✅, qwen3.5:0.8b selected as fastest model
- brain version constant bumped to 0.6.1 ✅
- Scout: updated to v0.0.5 (pyproject.toml) with ddgs 9.x compatibility
- Leg 1: get_impact no overflow (BUG-NEW-03 fixed ✅), search "BFS carver" now returns 2 hits
- Compact format + detail_level tiers implemented in new binary (not testable in this session)

### What Needs Fixing Before Next Test Run

1. **BUG-I-NEW-A** (BLOCKER): Update `ollamaRequest` to use `think: false` field for Ollama ≥0.6 API. Remove prompt prefix approach. This single fix recovers L2-B, L2-C, L2-E, L2-G, L3-D.
2. **BUG-SESSION-01**: Document that `synapses.json` with scout+brain config MUST exist at MCP root BEFORE starting Claude Code. Add to setup docs.
3. **BUG-L1G-01**: validate_plan should resolve relative file paths against the repo root.
4. **BUG-I-NEW-B**: Update test plan enrich API fields (`root_node_id`, `root_name`, etc).
5. **BUG-S-NEW-D**: Restore `source` field in fetch response.
...
| CONFIG-01  | Medium   | [FIXED] Parent-dir config walk discovery merged                                  |
...
_Update this section after each major version test._
