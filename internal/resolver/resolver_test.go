...
func TestResolveCallEdges_QualifiedUnknownAliasNoLongerResolvesCrossPackage(t *testing.T) {
	// Sprint 28: cross-package qualified fallback removed. An unknown alias
	// should no longer resolve across packages — only same-package lookup.
	g := graph.New("testrepo")
	...
	g.AddCallSite(graph.CallSite{
		CallerID:   callerFn,
		CallerFile: "main.go",
		PkgAlias:   "unknown", // not a real import alias
		FuncName:   "Connect",
	})

	n := resolver.ResolveCallEdges(g)
	if n != 0 {
		t.Error("expected no CALLS edge — unknown alias should not resolve cross-package (Sprint 28)")
	}
}
