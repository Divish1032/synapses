...
// ── discover_tools findability ─────────────────────────────────────────────────

// TestDiscoverTools_FindsGetEdgeTypes verifies that relevant natural-language
// queries surface get_edge_types via discover_tools. Before the fix, this tool
// was absent from toolCatalog and would never appear in any discover_tools result.
func TestDiscoverTools_FindsGetEdgeTypes(t *testing.T) {
	s := newTestServer(t)

	queries := []string{
		"edge types",
		"BFS weights",
		"what are the graph relationships",
		"edge catalog",
		"traversal weights",
	}
	...
	for _, q := range queries {
		t.Run(q, func(t *testing.T) {
			res, err := s.handleDiscoverTools(ctx, callTool(map[string]any{"query": q}))
			if err != nil {
			...
			for _, raw := range matches {
				...
				if entry["name"] == "get_edge_types" {
					found = true
					break
				}
			}
			if !found {
				t.Errorf("query %q: get_edge_types not in matches: %v", q, matches)
			}
		})
	}
}

// TestDiscoverTools_GetEdgeTypesInCatalogOverview verifies that empty-query
// discover_tools (catalog overview) includes get_edge_types in its categories.
func TestDiscoverTools_GetEdgeTypesInCatalogOverview(t *testing.T) {
	s := newTestServer(t)
	...
	cats, ok := m["categories"].(map[string]any)
	if !ok {
		t.Fatalf("expected categories map, got %T", m["categories"])
	}

	found := false
outer:
	for _, toolList := range cats {
		tools, ok := toolList.([]any)
		if !ok {
			continue
		}
	...
	if !found {
		t.Error("get_edge_types not found in any discover_tools category overview")
	}
}
