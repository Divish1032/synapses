...
// ── suggestToolsForChanges ────────────────────────────────────────────────────

func TestSuggestToolsForChanges_EmptyEvents(t *testing.T) {
	suggestions := suggestToolsForChanges([]changeEntry{})
	if len(suggestions) == 0 {
		t.Error("expected default suggestions for empty events")
	}
...
func TestSuggestToolsForChanges_SingleFile(t *testing.T) {
	events := []changeEntry{
		{File: "pkg/auth/auth.go", At: "10:00:00"},
	}
	suggestions := suggestToolsForChanges(events)
	if len(suggestions) == 0 {
...
func TestSuggestToolsForChanges_MultipleDifferentFiles(t *testing.T) {
	// Should cap at 2 file suggestions.
	events := []changeEntry{
		{File: "pkg/auth/auth.go", At: "10:00:00"},
		{File: "pkg/api/handler.go", At: "10:01:00"},
		{File: "pkg/db/db.go", At: "10:02:00"},
	}
	suggestions := suggestToolsForChanges(events)
	// Count get_file_context suggestions — should be limited to 2.
	count := 0
	for _, s := range suggestions {
		if s.Tool == "get_file_context" {
			count++
		}
	}
	if count > 2 {
		t.Errorf("expected at most 2 get_file_context suggestions, got %d", count)
	}
}

func TestSuggestToolsForChanges_DuplicateFiles(t *testing.T) {
	// Same file twice — should only count once.
	events := []changeEntry{
		{File: "pkg/auth/auth.go", At: "10:00:00"},
		{File: "pkg/auth/auth.go", At: "10:01:00"},
	}
	suggestions := suggestToolsForChanges(events)
	count := 0
	for _, s := range suggestions {
	...
	if count > 1 {
		t.Errorf("expected at most 1 get_file_context for duplicate file, got %d", count)
	}
}

func TestSuggestToolsForChanges_EmptyFilename(t *testing.T) {
	// File with empty path should be skipped for suggestions.
	events := []changeEntry{
		{File: "", At: "10:00:00"},
	}
	suggestions := suggestToolsForChanges(events)
	for _, s := range suggestions {
		if s.Tool == "get_file_context" && s.Reason == "" {
			t.Error("unexpected get_file_context with empty file")
		}
	}
}
...
func TestHandleGetWorkingState_WithWindowMinutes(t *testing.T) {
	s := newTestServer(t)
	req := callTool(map[string]any{"window_minutes": float64(30)})
	result, err := s.handleGetWorkingState(ctx, req)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	m := mustResult(t, result, nil)
	if wm, _ := m["window_minutes"].(float64); wm != 30 {
		t.Errorf("expected window_minutes=30, got %v", wm)
	}
}
...
// ── handleGetWorkingState (with real root directory) ──────────────────────────

func TestHandleGetWorkingState_WithRealRoot(t *testing.T) {
	root := t.TempDir()
	// Write a JSON config, a Makefile (configPatterns match), and a .go file
	// (non-config, covers the !isConfig continue path).
	for _, name := range []string{"synapses.json", "Makefile", "main.go"} {
	...
	req := callTool(map[string]any{"window_minutes": float64(60)})
	result, err := s.handleGetWorkingState(ctx, req)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
