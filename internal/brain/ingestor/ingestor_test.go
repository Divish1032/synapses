...
// --- deterministicSummary fast path ---

func TestDeterministicSummary_TestHelper(t *testing.T) {
	t.Parallel()
	summary, tags, ok := deterministicSummary(Request{
		NodeName: "TestAuthService_Login",
		NodeType: "function",
		Package:  "auth_test",
	})
	if !ok {
		t.Fatal("expected deterministic hit for test helper")
	}
	if summary == "" {
...
func TestDeterministicSummary_MockType(t *testing.T) {
	t.Parallel()
	_, tags, ok := deterministicSummary(Request{
		NodeName: "MockStore",
		NodeType: "struct",
		Package:  "store",
	})
	if !ok {
		t.Fatal("expected deterministic hit for mock type")
	}
	if len(tags) != 1 || tags[0] != "test" {
		t.Errorf("tags = %v, want [test]", tags)
	}
...
func TestDeterministicSummary_GeneratedCode(t *testing.T) {
	t.Parallel()
	_, tags, ok := deterministicSummary(Request{
		NodeName: "ParseAST",
		NodeType: "function",
	...
	if !ok {
		t.Fatal("expected deterministic hit for generated code")
	}
	if len(tags) != 1 || tags[0] != "generated" {
		t.Errorf("tags = %v, want [generated]", tags)
	}
}

func TestDeterministicSummary_TrivialGetter(t *testing.T) {
	t.Parallel()
	_, tags, ok := deterministicSummary(Request{
		NodeName: "GetName",
		NodeType: "method",
		Package:  "model",
		Code:     "func (m *Model) GetName() string { return m.name }",
	})
	if !ok {
		t.Fatal("expected deterministic hit for trivial getter")
	}
	if len(tags) != 1 || tags[0] != "util" {
		t.Errorf("tags = %v, want [util]", tags)
	}
}

func TestDeterministicSummary_InitFunc(t *testing.T) {
	t.Parallel()
	_, tags, ok := deterministicSummary(Request{
		NodeName: "init",
		NodeType: "function",
	...
	if !ok {
		t.Fatal("expected deterministic hit for init()")
	}
	if len(tags) != 1 || tags[0] != "config" {
		t.Errorf("tags = %v, want [config]", tags)
	}
}

func TestDeterministicSummary_NonTrivial_ReturnsNoHit(t *testing.T) {
	t.Parallel()
	_, _, ok := deterministicSummary(Request{
	...
	if ok {
		t.Error("expected no deterministic hit for non-trivial struct")
	}
}

func TestDeterministicSummary_LongGetter_NoHit(t *testing.T) {
	t.Parallel()
	// Getter with complex body (>80 chars) should NOT be fast-pathed.
	_, _, ok := deterministicSummary(Request{
		NodeName: "GetConfig",
		NodeType: "function",
		Package:  "config",
		Code:     "func GetConfig() *Config { cfg := loadFromDisk(); validate(cfg); applyDefaults(cfg); return cfg } // this is a complex getter",
	})
	if ok {
		t.Error("expected no deterministic hit for complex getter (>80 chars)")
	}
}
