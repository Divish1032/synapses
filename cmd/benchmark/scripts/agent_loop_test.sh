...
timeout $TIMEOUT claude -p "$PROMPT" \
  ...
  --verbose \
  < /dev/null 2>/dev/null > "$BASELINE_OUT" || true
...
BASELINE_ENTITIES=$(echo "$BASELINE_RESULT" | grep -c "^[0-9]" || echo "0")
...
SYNAPSES_OUT=$(mktemp)
timeout $TIMEOUT claude -p "$SYNAPSES_PROMPT" \
  ...
  --output-format stream-json \
  --verbose \
  < /dev/null 2>/dev/null > "$SYNAPSES_OUT" || true
...
SYNAPSES_ENTITIES=$(echo "$SYNAPSES_RESULT" | grep -c "^[0-9]" || echo "0")
